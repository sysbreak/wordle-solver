...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if response.status().is_success() {
        ...
        loop {
                ...
                if filtered_words.len() == 1 {
                    ...
                    current_guess = match best_guess_candidate {
                        ...
                        None => { // Should not happen if filtered_words is not empty here
                            println!("Error: Could not determine a guess. Exiting.");
                            break;
                        }
                    };
            ...
            print!("Enter feedback for '{}' (Green = @, Yellow = #, Gray = ?): ", current_guess);
            io::stdout().flush()?;
            let mut feedback_input = String::new();
            io::stdin().read_line(&mut feedback_input)?;
            let feedback_input_trimmed = feedback_input.trim();
            ...
            if !valid_feedback_chars {
                println!("Error: Feedback contains invalid characters. Use only '@' (Green), '#' (Yellow), '?' (Gray). Please try again for guess '{}'.", current_guess);
